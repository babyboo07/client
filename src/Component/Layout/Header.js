...
function Header(props) {
  ...
  useEffect(() => {
    setAcc(
      localStorage.getItem("googleAcc") || localStorage.getItem("accessToken")
    );
  }, [acc]);
  return (
    <nav className="grow pt-3 bg-white w-full shadow-xl">
      <div className="flex justify-between">
        ...
        {acc != null || acc != undefined ? (
          <div className="d-flex">
            ...
            <div className="relative inline-block text-left">
              <div>
                ...
                >
                  {JSON.parse(acc).imageUrl ? (
                    <Avatar alt="userImg" sx={{ width: 28, height: 28 }} src={JSON.parse(acc).imageUrl} />
                  ) : (
          ...
          <div>
            <a href="/login" className="no-underline pr-4">
              Login
            </a>
          </div>
        )}
